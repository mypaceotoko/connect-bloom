...
| `likedUserIds` | 自分が送ったいいね | `likes.from_user_id` / `likes.to_user_id` | まだlocalStorage |
| `receivedLikeUserIds` | 受け取ったいいね | `likes` | まだlocalStorage |
| `matchedUserIds` | 成立済みマッチ | `matches` | まだlocalStorage |
| `messagesByMatchId` | マッチごとのDM | `messages` | まだlocalStorage |
